...
@Description: This program takes the textfile 'input.txt' and prints out all the words in the textfile
              and then prompts the user to enter one of the listed words. Then, the program will output
              every sentence that word is in along with where that sentence appears in the text file.
@Date: 2023/01/25
@Updated: 2023/01/26
...
def word_list():
    ...
    for letter in text:
        if letter == '.':
            text_without_punct = text.replace(letter, " ")
...
def sentence_sorter():
    """  
    This function prompt the user to enter a word from the list_of_words
    ...
    #A second variable 'word_check' is created to be  
    #used as the verification for the user's input 
    word_check = text
    for letter in word_check:
        if letter == '.':
            word_check = word_check.replace(letter, " ")
    word_check = word_check.lower().split()
